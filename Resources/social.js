...
var OAuthAdapter = function(pConsumerSecret, pConsumerKey, pSignatureMethod) {
	...
	this.send = function(options, callback) {
		...
		var message = createMessage(pUrl, options.method);
		...
		if(options.method === 'GET') {
			...
			client.open(options.method, OAuth.addToURL(pUrl, message.parameters));
...
var supportedSites = {
	...
	dropbox : {
		...
		authorize : 'https://www.dropbox.com/1/oauth/authorize?oauth_callback=http://www.clearlyinnovative.com/oAuth.html&',
		update : 'https://api.twitter.com/1/statuses/update.json'
	}
};
...
exports.create = function(settings) {
	...
	var adapter = new OAuthAdapter(settings.consumerSecret, settings.consumerKey, 'PLAINTEXT');
	this.OAuthAdapter = adapter;
