...
- (void)boot
{
	NSLog(@"[INFO] %@/%@ (%s.4cb7aff)",TI_APPLICATION_NAME,TI_APPLICATION_VERSION,TI_VERSION_STR);
...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kdropboxapiUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kdropboxapiUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
